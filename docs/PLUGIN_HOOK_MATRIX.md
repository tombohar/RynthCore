...
This matrix tracks clean-room hook targets that can be implemented from the AC client decompile without inspecting third-party plugin binaries.
...
| `OnCombatModeChange` | `ClientCombatSystem.cpp` | Confirmed | Combat mode callback exists. |
...
| `OnUIInitialized` | `ClientUISystem::OnStartup` and `APIManager::SetUIReady(1)` | Investigating | Interface exists; fire site not yet confirmed. |
...
| Selection get/set | `APIManager__IAsheronsCallImpl.cpp` | Confirmed | Useful for plugin wrappers. |
...
2. Expose the confirmed `IAsheronsCall` subset through RynthCore-native APIs.
