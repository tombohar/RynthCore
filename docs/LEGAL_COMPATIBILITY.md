...
This project is building a new NexCore-native plugin system first, with clean-room compatibility shims only where they are safe and useful.
...
- U.S. law includes an interoperability exception in `17 U.S.C. 1201(f)`, but NexCore should treat that as a narrow backstop instead of the core project strategy.
...
- Do not market NexCore as an official Decal replacement or imply endorsement.
- Prefer source/API compatibility over binary cloning.
- Prefer NexCore-native plugins over legacy binary compatibility whenever possible.
...
- NexCore code and user-provided test plugins.
- Public legal/license text published by the relevant project owners.

## Current Safe First Step

The safest near-term compatibility work is implementing Decal-shaped lifecycle and callback seams inside NexCore, starting with events such as `OnLoginComplete`, chat events, target changes, and trade events, without inspecting or copying Decal or VTank implementation details.
