...
This file tracks NexCore hook targets against the user's real AC client binary, with the AC client decompile used for semantics and the live binary used as the source of truth for hook placement.
...
- Prefer hooking the AC client function that already implements the Decal-shaped behavior over lower-level memory access.
...
| --- | --- | --- | --- | --- |
| `OnLoginComplete` | `CPlayerSystem::SendLoginCompleteNotification` in `CPlayerSystem.cpp:4017-4031` | Pattern-scan the containing function using the unique `SetDisplayInventory(1)` plus `BroadcastGlobalMessage(..., 0xB, 0)` sequence, then hook the containing `thiscall` function. | Live in NexCore | P0 |
...
| Trade accept/decline/close/clear | `ClientTradeSystem.cpp` | Hook the direct trade-system wrappers that already invoke the corresponding Decal callback. | Decompiled seam confirmed, binary anchor still needed | P1 |

## Host Call Surface To Expose Through NexCore

These are not plugin callback hooks, but they are part of the Decal-shaped host API surface we should expose once the callback layer is in place.
...
| `IsStandingStill`, `StopCompletely`, `TurnToHeading`, `SetAutoRun` | `APIManager__IAsheronsCallImpl.cpp` | Movement helper layer. Some of this already overlaps with existing NexCore action hooks. | P0 |
...
5. Expose the confirmed `IAsheronsCall` subset through NexCore-native host APIs.
...
- For this reason, future hook work should be tracked here first against the fingerprint above, then implemented in NexCore with pattern scanning rather than fixed addresses.
