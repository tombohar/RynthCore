...
namespace NexSuite.Plugins.RynthAi
{
    public class MetaUI
    {
        ...
        private readonly List<string> _navFiles; // Passed in so we can embed routes

        public Action OnSettingsChanged;
        public Action OnMetaRulesUpdated;

        // --- META STATE ---
        private bool _showMetaEditor = false;
        private MetaRule _editingRule = new MetaRule();
        ...
        public MetaUI(UISettings settings, List<string> navFiles)
        {
            _settings = settings;
            _navFiles = navFiles;
        }
        ...
        public void Render()
        {
            if (ImGui.BeginTable("MetaRulesTable", 6, ImGuiTableFlags.BordersInnerH | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY, new Vector2(0, -35)))
            {
                ...
                foreach (var group in groupedRules)
                {
                    ...
                    for (int i = 0; i < stateRules.Count; i++)
                    {
                        ...
                        if (ImGui.Button($"X##delm{globalIdx}")) { _settings.MetaRules.Remove(rule); OnSettingsChanged?.Invoke(); }
                        ImGui.PopStyleColor();

                        ImGui.TableNextColumn();
                        if (ImGui.Selectable($"{rule.State}##edit{globalIdx}", false, ImGuiSelectableFlags.SpanAllColumns))
                        {
                            _editingRule = new MetaRule { State = rule.State, Condition = rule.Condition, ConditionData = rule.ConditionData, Action = rule.Action, ActionData = rule.ActionData };
                            if (rule.Children != null)
                            {
                                string json = Newtonsoft.Json.JsonConvert.SerializeObject(rule.Children);
                                _editingRule.Children = Newtonsoft.Json.JsonConvert.DeserializeObject<List<MetaRule>>(json);
                            }
                        ...
                        ImGui.TableNextColumn();
                        ImGui.Text(rule.Condition.ToString() + (string.IsNullOrEmpty(rule.ConditionData) ? "" : $": {rule.ConditionData}"));

                        ImGui.TableNextColumn();
                        if (rule.Action == MetaActionType.All)
                        {
                            int childCount = rule.Children?.Count ?? 0;
                            ImGui.TextColored(new Vector4(1, 1, 0, 1), $"All: [{childCount} actions]");
                        }
                        else if (rule.Action == MetaActionType.EmbeddedNavRoute && !string.IsNullOrEmpty(rule.ActionData) && rule.ActionData.Contains(";"))
                        {
                        ...
                        {
                            ImGui.Text(rule.Action.ToString() + (string.IsNullOrEmpty(rule.ActionData) ? "" : $": {rule.ActionData}"));
                        }
            ...
            if (ImGui.BeginPopupModal("CreateStatePopup", ref _showMetaEditor, ImGuiWindowFlags.AlwaysAutoResize))
            {
        ...
        {
            if (_showMetaEditor) ImGui.OpenPopup("Edit Meta Rule");

            ImGui.SetNextWindowSize(new Vector2(650, 450), ImGuiCond.Appearing);

            if (ImGui.BeginPopupModal("Edit Meta Rule", ref _showMetaEditor, ImGuiWindowFlags.NoCollapse))
            {
                ...
                ImGui.BeginChild("ConditionPane", new Vector2(halfWidth, -35), true);
                ...
                ImGui.BeginChild("ActionPane", new Vector2(0, -35), true);
        ...
        private void RenderRuleActionNode(MetaRule rule, string idContext)
        {
            ...
            if (ImGui.Combo("##ActionType", ref actionIdx, _metaActionNames, _metaActionNames.Length))
            {
                ...
                else if (rule.Children != null) rule.Children.Clear();
            }
            ...
            if (rule.Action == MetaActionType.All)
            {
                ...
                {
                    rule.Children.Add(new MetaRule { Action = MetaActionType.ChatCommand });
                }
                ImGui.PopStyleColor();
                ...
                for (int i = 0; i < rule.Children.Count; i++)
                {
                    ...
                    ImGui.PopID();
                    if (i < rule.Children.Count - 1) ImGui.Separator();
                }
                ImGui.Unindent(20f);
            }
            else
            {
                switch (rule.Action)
                {
                    ...
                    case MetaActionType.CallMetaState:
                        ...
                        ImGui.SetNextItemWidth(-1);
                        if (ImGui.InputTextWithHint("##State", "Target State Name...", ref state, 64))
                            rule.ActionData = state;
                        break;

                    case MetaActionType.EmbeddedNavRoute:
                        if (ImGui.BeginCombo("##NavSelect", string.IsNullOrEmpty(rule.ActionData) ? "Select Route..." : rule.ActionData))
                        {
                    ...
                    case MetaActionType.SetWatchdog:
                        ...
                        if (ImGui.BeginCombo("##WDStateCombo", wdState))
                        {
                            ...
                            {
                                if (ImGui.Selectable(s, wdState == s))
                                {
                                    wdState = s;
                                }
                            }
        ...
        public void RenderAsWindow(ref bool open)
        {
            ...
            if (ImGui.Begin("Macro Rules##RynthAiMeta", ref open))
                Render();
            ImGui.End();
        }

        private void ClearConditionDataIfNeeded(MetaRule rule)
        {
            switch (rule.Condition)
            {
                case MetaConditionType.Always:
