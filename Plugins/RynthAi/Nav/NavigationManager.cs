...
namespace NexSuite.Plugins.RynthAi
{
    ...
    public class NavigationManager
    {
        ...
        // ── Heading thresholds (from settings — tunable in Advanced → Navigation) ──
        ...
        private double BigTurnExit    => Math.Max(1.0, Math.Min(_settings.NavResumeTurnAngle, BigTurnEnter - 1));
        ...
        private bool _tier2ModeLoggedOnce = false;

        public void ProcessNavigation(bool isFighting)
        {
            // One-shot: dump UB's SmartBox pointer chain vs our pattern-scanned one
            if (_settings.MovementMode == 2 && !_tier2ModeLoggedOnce)
            {
                _tier2ModeLoggedOnce = true;
                Tier2Log.Log($"MODE 2 ACTIVE: macro={_settings.IsMacroRunning} " +
                             $"nav={_settings.EnableNavigation} state={_settings.CurrentState}");
            }
            ...
            if (_settings.MovementMode == 2 && Tier2MovementHelper.IsInitialized)
            {
                try
                {
                    Tier2Log.Log("T2 CHECK: calling GetPlayerCellId");
                    uint _t2cell = Tier2MovementHelper.GetPlayerCellId();
                    Tier2Log.Log($"T2 CHECK: cell=0x{_t2cell:X8} outdoor={(_t2cell & 0xFFFF) < 0x100}");
                    Tier2MovementHelper.LogPointerChain();  // after cellId succeeds
                    if ((_t2cell & 0xFFFF) < 0x100)
                    {
                ...
                {
                    Tier2Log.Log($"T2 CHECK CRASHED: {ex}");
            ...
            {
                _lastDebugPrint = DateTime.Now;
                string turnDir = absError < DeadZone ? "NONE" : (error > 0 ? "RIGHT" : "LEFT");
                double ox = 0, oy = 0;
            ...
            CoordsObject pos, double distYards)
        {
            ...
            if (_prevDist < ArrivalYards * SweepMult && distYards > _prevDist + 0.3)
            {
                _prevDist = double.MaxValue;
                CancelTier2();
                Advance(route);
            ...
            if (idx != _tier2TargetIdx)
            {
                ...
                needsIssue = true;
                     ...
                     (DateTime.Now - _tier2IssuedAt).TotalMilliseconds > _settings.T2ReissueMs)
            {
        ...
        private void IssueTier2Move(NavPoint pt, CoordsObject decalPos)
        {
            try
            {
                ...
                if ((playerCell & 0xFFFF) >= 0x100)
                    return;  // indoor — caller shouldn't have sent us here
                ...
                double maxRange = Math.Max(10.0, _settings.T2MaxRangeYd);
                if (deltaDist > maxRange)
                {
                    double scale = maxRange / deltaDist;
                    deltaEW *= scale;
                    deltaNS *= scale;
                }
                ...
                // Player origin is LANDBLOCK-relative (0–192 yards), NOT cell-relative
                double globalEW = pLbEW * 192.0 + playerX + deltaEW;
                double globalNS = pLbNS * 192.0 + playerY + deltaNS;
                ...
                // Target origin: landblock-relative (0–192), same as how AC stores it
                double inLbEW = globalEW - tLbEW * 192.0;
                double inLbNS = globalNS - tLbNS * 192.0;

                // Cell index is derived from position but origin stays landblock-relative
                int tColEW = Math.Max(0, Math.Min(7, (int)Math.Floor(inLbEW / 24.0)));
                int tRowNS = Math.Max(0, Math.Min(7, (int)Math.Floor(inLbNS / 24.0)));

                float localX = Math.Max(0.5f, Math.Min(191.5f, (float)inLbEW));
                float localY = Math.Max(0.5f, Math.Min(191.5f, (float)inLbNS));
                float localZ = playerZ;

                int cellIdx = tRowNS * 8 + tColEW + 1;
                uint targetCell = (uint)((tLbEW << 24) | (tLbNS << 16) | cellIdx);

                int maxLb = Math.Max(1, _settings.T2MaxLandblocks);
                if (Math.Abs(tLbEW - pLbEW) > maxLb || Math.Abs(tLbNS - pLbNS) > maxLb)
                {
                    Tier2Log.Log($"SKIP: lb ({tLbEW},{tLbNS}) too far from ({pLbEW},{pLbNS})");
                    return;
                }

                Tier2Log.Log($"ISSUE: 0x{targetCell:X8} local=({localX:F1},{localY:F1},{localZ:F1}) " +
                             $"delta=({deltaEW:F0},{deltaNS:F0})yd spd={_settings.T2Speed} dist={_settings.T2DistanceTo} walk={_settings.T2WalkWithinYd}");

                uint result = Tier2MovementHelper.MoveToPosition(
                    targetCell, localX, localY, localZ,
                    Math.Max(0.1f, _settings.T2Speed),
                    Math.Max(0.1f, _settings.T2DistanceTo),
                    Math.Max(0f, _settings.T2WalkWithinYd));

                _tier2Active = (result == 0);
                _tier2IssuedAt = DateTime.Now;
                Tier2Log.Log($"RESULT: {result}");
            }
            catch (Exception ex)
            {
                Tier2Log.Log($"ISSUE EXCEPTION: {ex}");
            }
        }
