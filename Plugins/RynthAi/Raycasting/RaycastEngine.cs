...
namespace NexSuite.Plugins.RynthAi.Raycasting
{
    ...
    public static class RaycastEngine
    {
        ...
        public static bool IsLinearPathBlocked(Vector3 origin, Vector3 target, List<BoundingVolume> geometry)
        {
            ...
            {
                // Skip doors (state is unreliable, as per VTank's approach)
                if (volume.IsDoor)
                    continue;
                ...
                {
                    // Ignore geometry within 2m of the player — prevents false blocks
                    // from walls/floors the player is standing on or touching.
                    // Only count geometry that's actually BETWEEN the player and target.
                    if (hitDist >= 2.0f && hitDist < distanceToTarget)
                    {
