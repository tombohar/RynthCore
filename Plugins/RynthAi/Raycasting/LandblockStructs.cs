                ...
                using (var reader = new BinaryReader(ms))
                {
                    ...
                    for (int p = 0; p < placementsCount; p++)
                    {
                        ...
                        for (int part = 0; part < numParts; part++)
                        {
                            ...
                            ms.Seek(28, SeekOrigin.Current); // Skip Frame
                        }
        ...
        public bool Unpack(byte[] data)
        {
            ...
            try
            {
                ...
                using (var reader = new BinaryReader(ms))
                {
                    ...
                    ms.Seek(numSurfaces * 4, SeekOrigin.Current); // uint32 per surface

                    // Read vertex count
                    if (ms.Position + 4 > ms.Length) return false;
                    uint numVertices = reader.ReadUInt32();
                    if (numVertices > 100000) return false;
                    ...
                    for (uint i = 0; i < numVertices; i++)
                    {
                        ...
                        // Skip normal (3 floats) + UV data
                        if (ms.Position + 12 <= ms.Length)
                            ms.Seek(12, SeekOrigin.Current); // Skip normal

                        // UV data varies; skip based on flags
                        if ((Flags & 0x04) != 0 && ms.Position + 8 <= ms.Length)
                            ms.Seek(8, SeekOrigin.Current); // Skip UV
                    }
