...
namespace NexSuite.Plugins.RynthAi.Raycasting
{
    ...
    public class GeometryLoader : IDisposable
    {
        ...
        /// The landblockId should be the Decal Landcell value (format: 0xXXYYnnnn).
        /// </summary>
        public List<BoundingVolume> GetLandblockGeometry(uint landcellId)
        {
            ...
            _landblockCache[landblockKey] = volumes;
            return volumes;
        }
        ...
        private int ParseLandblockObjects(byte[] data, uint landblockKey, List<BoundingVolume> volumes)
        {
            ...
            if (buildingOffset + 4 <= data.Length)
            {
                ...
                for (int b = 0; b < numBuildings && bOff + 36 <= data.Length; b++)
                {
                    ...
                    var bVolumes = GetSetupBoundingVolumes(modelId);
        ...
        private void LoadIndoorCells(uint landblockKey, List<BoundingVolume> volumes)
        {
            ...
            for (uint cellNum = 0x0100; cellNum <= 0xFFFD; cellNum++)
            {
                ...
                {
                    consecutiveMisses++;
                    if (consecutiveMisses > 10) break; // No more cells after 10 gaps
                    continue;
                }
                consecutiveMisses = 0;
