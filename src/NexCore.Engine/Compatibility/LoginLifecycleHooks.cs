...
internal static class LoginLifecycleHooks
{
    ...
    public static void Initialize(Action<string>? log = null)
    {
        ...
        try
        {
            ...
            if (imageSize != ExpectedImageSize)
                log?.Invoke($"Compat: login lifecycle hook using unverified acclient image size 0x{imageSize:X} (expected 0x{ExpectedImageSize:X}).");
            ...
            if (callSiteOff < 0)
            {
                ...
                log?.Invoke($"Compat: login lifecycle hook failed - {_statusMessage}");
                return;
            }
            ...
            _originalSendLoginCompleteNotification = Marshal.GetDelegateForFunctionPointer<SendLoginCompleteNotificationDelegate>(originalPtr);
        ...
        catch (Exception ex)
        {
            ...
            log?.Invoke($"Compat: login lifecycle hook failed - {ex.Message}");
        }
    }
    ...
    public static void Poll()
    {
        ...
        {
            _waitingLogged = true;
            EntryPoint.Log("Compat: login lifecycle awaiting SendLoginCompleteNotification.");
        }
    }
    ...
    private static void SignalLoginComplete(string source)
    {
        ...
        EntryPoint.Log($"Compat: login complete observed via {source}.");
