...
internal static class MultiClientHooks
{
    ...
    public static void Initialize(Action<string>? log = null)
    {
        ...
        {
            log?.Invoke($"Compat: multi-client bypass skipped - {_statusMessage}");
            return;
        }
        ...
        if (!AcClientModule.TryReadTextSection(out AcClientTextSection textSection, log))
        {
            ...
            log?.Invoke($"Compat: multi-client hook failed - {_statusMessage}");
            return;
        }
        ...
        try
        {
            if (textSection.ImageSize != ExpectedImageSize)
                log?.Invoke($"Compat: multi-client hook using unverified acclient image size 0x{textSection.ImageSize:X} (expected 0x{ExpectedImageSize:X}).");
            ...
            TryInstallIsAlreadyRunningHook(textSection, log);
            TryInstallOpenDataFileHook(textSection, log);
        ...
        catch (Exception ex)
        {
            ...
            log?.Invoke($"Compat: multi-client hook failed - {ex.Message}");
        }
    }

    private static void TryInstallIsAlreadyRunningHook(AcClientTextSection textSection, Action<string>? log)
    {
        int funcOff = IsAlreadyRunningVa - textSection.TextBaseVa;
        if (funcOff < 0 || funcOff + IsAlreadyRunningSignature.Length > textSection.Bytes.Length)
        {
            log?.Invoke($"Compat: multi-client hook unavailable - Client::IsAlreadyRunning VA 0x{IsAlreadyRunningVa:X8} is outside the readable text window.");
            return;
        }
        ...
        for (int i = 0; i < IsAlreadyRunningSignature.Length; i++)
        {
            ...
            if (expected.HasValue && textSection.Bytes[funcOff + i] != expected.Value)
            {
                log?.Invoke($"Compat: multi-client hook unavailable - Client::IsAlreadyRunning signature mismatch at 0x{IsAlreadyRunningVa + i:X8}.");
                return;
            }
        }
        ...
        _originalIsAlreadyRunning = Marshal.GetDelegateForFunctionPointer<IsAlreadyRunningDelegate>(originalPtr);

        IsAlreadyRunningInstalled = true;
        log?.Invoke($"Compat: multi-client hook ready - IsAlreadyRunning=0x{IsAlreadyRunningVa:X8}");
    }

    private static void TryInstallOpenDataFileHook(AcClientTextSection textSection, Action<string>? log)
    {
        ...
        {
            log?.Invoke($"Compat: multi-client data-file hook unavailable - CLBlockAllocator::OpenDataFile VA 0x{OpenDataFileVa:X8} is outside the readable text window.");
            return;
        }
        ...
        for (int i = 0; i < OpenDataFileSignature.Length; i++)
        {
            ...
            if (expected.HasValue && textSection.Bytes[funcOff + i] != expected.Value)
            {
                log?.Invoke($"Compat: multi-client data-file hook unavailable - CLBlockAllocator::OpenDataFile signature mismatch at 0x{OpenDataFileVa + i:X8}.");
                return;
            }
        }
        ...
        IntPtr originalPtr = MinHook.Hook(_openDataFileAddress, detourPtr);
        _originalOpenDataFile = Marshal.GetDelegateForFunctionPointer<OpenDataFileDelegate>(originalPtr);

        OpenDataFileInstalled = true;
        log?.Invoke($"Compat: multi-client data-file hook ready - CLBlockAllocator::OpenDataFile=0x{OpenDataFileVa:X8}");
    }
