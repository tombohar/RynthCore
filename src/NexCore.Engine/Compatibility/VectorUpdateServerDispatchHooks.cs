...
internal static class VectorUpdateServerDispatchHooks
{
    ...
    public static void Initialize(Action<string>? log = null)
    {
        ...
        if (!PatternScanner.VerifyBytes(textSection.Bytes, funcOff, DispatchSbVectorUpdateSignature))
        {
            ...
            log?.Invoke($"Compat: vector-update hook failed - {_statusMessage}");
            return;
        }
        ...
        try
        {
            unsafe
            {
                ...
                _originalDispatchSbVectorUpdatePtr = MinHook.Hook(_targetAddress, (IntPtr)pDetour);
            }

            IsInstalled = true;
            _statusMessage = $"Hooked CM_Physics::DispatchSB_VectorUpdate @ 0x{_targetAddress.ToInt32():X8}.";
            log?.Invoke($"Compat: vector-update hook ready - DispatchSB_VectorUpdate=0x{_targetAddress.ToInt32():X8}");
        }
        catch (Exception ex)
        {
            _statusMessage = ex.Message;
            log?.Invoke($"Compat: vector-update hook failed - {ex.Message}");
        }
    }
    ...
    private static unsafe uint DispatchSbVectorUpdateDetour(IntPtr smartBoxPtr, IntPtr blob)
    {
        ...
        {
            EntryPoint.Log(
                $"Compat: vector update #{count} opcode=0x{info.Opcode:X8} rawId=0x{info.RawObjectId:X8} size={info.BlobSize} status={status}");
        }
