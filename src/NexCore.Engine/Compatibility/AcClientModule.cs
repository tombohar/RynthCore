...
internal static class AcClientModule
{
    ...
    public static bool TryReadTextSection(out AcClientTextSection textSection, Action<string>? log = null)
    {
        textSection = default;
        ...
        try
        {
            ...
            if (moduleBase == IntPtr.Zero)
            {
                log?.Invoke($"Compat: acclient.exe module not found (error {Marshal.GetLastWin32Error()}).");
                return false;
            }

            if (!TryReadImageSize(moduleBase, out int imageSize))
            {
                log?.Invoke("Compat: failed to read acclient.exe PE headers.");
                return false;
            }

            log?.Invoke($"Compat: acclient.exe base=0x{moduleBase.ToInt32():X8}, size=0x{imageSize:X}");

            int rawTextSize = imageSize - DefaultTextRva;
            int textSize = rawTextSize;
            if (textSize <= 0 || textSize > SuspiciousTextSectionBytes)
            {
                log?.Invoke($"Compat: suspicious .text size 0x{rawTextSize:X}; clamping scan window.");
            ...
            if (textSize <= 0)
            {
                log?.Invoke("Compat: failed to derive a readable acclient .text window.");
                return false;
            }
        ...
        {
            log?.Invoke($"Compat: failed to read acclient text section - {ex.Message}");
            return false;
        }
    }
