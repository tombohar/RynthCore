...
internal static class ChatCallbackHooks
{
    ...
    public static void Initialize(Action<string>? log = null)
    {
        ...
        try
        {
            ...
            if (EnableIncomingHook && !IncomingInstalled)
                IncomingInstalled = TryInstallIncomingHook(textSection, log, out incomingStatus);
            ...
            {
                if (TryResolveOutgoingOffset(textSection, incomingVerified, log, out int outgoingOff, out string outgoingResolution))
                {
                    try
                    {
                        ...
                        IntPtr outgoingOriginalPtr = MinHook.Hook(_outgoingAddress, outgoingPtr);
                        _originalOutgoingChat = Marshal.GetDelegateForFunctionPointer<OutgoingChatDelegate>(outgoingOriginalPtr);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        log?.Invoke($"Compat: outgoing chat hook unavailable - {ex.Message}");
                    }
                }
                else
                {
                    ...
                    if (!outgoingResolution.StartsWith("skipped", StringComparison.Ordinal))
                        log?.Invoke($"Compat: outgoing chat hook unavailable - {outgoingResolution}");
                }
            }
            ...
            if (!IncomingInstalled && !OutgoingInstalled)
            {
                ...
                log?.Invoke($"Compat: chat callback hook failed - {_statusMessage}");
                return;
            }

            _statusMessage = $"Ready. incoming={incomingStatus}, outgoing={outgoingStatus}.";
            log?.Invoke($"Compat: chat callback hooks ready - incoming={incomingStatus}, outgoing={outgoingStatus}");
        }
        catch (Exception ex)
        {
            _statusMessage = ex.Message;
            log?.Invoke($"Compat: chat callback hook failed - {ex.Message}");
        }
    }

    private static bool TryInstallIncomingHook(AcClientTextSection textSection, Action<string>? log, out string incomingStatus)
    {
        if (TryInstallPreferredIncomingHook(textSection, log, out incomingStatus))
            return true;

        if (CurrentIncomingMode is not IncomingChatMode.WrapperQueuedText and not IncomingChatMode.WrapperQueuedTextHostEat)
        {
            log?.Invoke("Compat: preferred incoming chat seam unavailable - falling back to wrapper path.");
            return TryInstallIncomingWrapperHook(textSection, log, out incomingStatus);
        }

        return false;
    }

    private static bool TryInstallPreferredIncomingHook(AcClientTextSection textSection, Action<string>? log, out string incomingStatus)
    {
        return CurrentIncomingMode switch
        {
            IncomingChatMode.AddTextToScrollHostEat => TryInstallIncomingAddTextHook(textSection, log, out incomingStatus),
            IncomingChatMode.WrapperQueuedText => TryInstallIncomingWrapperHook(textSection, log, out incomingStatus),
            IncomingChatMode.WrapperQueuedTextHostEat => TryInstallIncomingWrapperHook(textSection, log, out incomingStatus),
            _ => TryInstallIncomingWrapperHook(textSection, log, out incomingStatus)
        };
    }

    private static bool TryInstallIncomingWrapperHook(AcClientTextSection textSection, Action<string>? log, out string incomingStatus)
    {
        ...
        if (!PatternScanner.VerifyBytes(textSection.Bytes, incomingOff, IncomingChatWrapperSignature))
        {
            ...
            log?.Invoke($"Compat: incoming chat hook unavailable - {incomingStatus}");
            return false;
        }
        ...
        try
        {
            ...
            IntPtr incomingOriginalPtr = MinHook.Hook(_incomingAddress, incomingPtr);
            _originalIncomingChatWrapper = Marshal.GetDelegateForFunctionPointer<IncomingChatWrapperDelegate>(incomingOriginalPtr);
        ...
        catch (Exception ex)
        {
            ...
            log?.Invoke($"Compat: incoming chat hook unavailable - {ex.Message}");
            return false;
        }
    }

    private static bool TryInstallIncomingAddTextHook(AcClientTextSection textSection, Action<string>? log, out string incomingStatus)
    {
        ...
        try
        {
            unsafe
            {
                ...
                _originalIncomingChatAddTextPtr = MinHook.Hook(_incomingAddress, (IntPtr)pDetour);
            }
            incomingStatus = $"0x{_incomingAddress.ToInt32():X8} ({GetIncomingModeLabel(CurrentIncomingMode)})";
            return true;
        }
    ...
    private static int IncomingChatWrapperDetour(int flags, IntPtr text, int chatChannel)
    {
        ...
        {
            try { EntryPoint.Log($"Compat: incoming wrapper detour error - {ex.GetType().Name}: {ex.Message}"); } catch { }
    ...
    private static string? ReadIncomingChatLine(IntPtr pStringBase, uint chatType)
    {
        try
        {
            ...
            if (count <= 10)
                EntryPoint.Log($"Compat: incoming chat #{count} type={chatType} len={length}");
    ...
    private static int OutgoingChatDetour(IntPtr thisPtr, IntPtr text, uint commandSource)
    {
        ...
        {
            try { EntryPoint.Log($"Compat: outgoing chat detour error - {ex.GetType().Name}: {ex.Message}"); } catch { }
    ...
    private static string? ReadWidePString(IntPtr pStringBase)
    {
        ...
        // The wrapper path hands us the legacy PSRefBuffer-backed wide string shape.
        // Do not reuse this reader for ClientSystem::AddTextToScroll; Chorizite's
        // AddTextToScroll seam uses a different PStringBase<ushort> layout.
        IntPtr buffer = Marshal.ReadIntPtr(pStringBase);
        if (buffer == IntPtr.Zero)
            return null;

        IntPtr data = buffer + PsRefBufferWideDataOffset;
        if (data == IntPtr.Zero)
            return null;

        int length = 0;
        while (length < MaxIncomingChatChars && Marshal.ReadInt16(data, length * sizeof(char)) != 0)
            length++;
            ...
            if (fixedOff >= 0 && fixedOff < textSection.Bytes.Length)
            {
                ...
                else
                {
                    ...
                    if (IsLikelyExternalHookOpcode(entryByte))
                    {
                        ...
                        log?.Invoke($"Compat: outgoing chat entry at 0x{OutgoingChatVa:X8} appears prepatched (opcode 0x{entryByte:X2}) - skipping NexCore outgoing hook to avoid conflicting with another injector.");
                        return false;
                    }
                }
            }
