...
internal static class UpdateObjectServerDispatchHooks
{
    ...
    public static void Initialize(Action<string>? log = null)
    {
        ...
        if (!PatternScanner.VerifyBytes(textSection.Bytes, funcOff, DispatchSbUpdateObjectSignature))
        {
            ...
            log?.Invoke($"Compat: update-object hook failed - {_statusMessage}");
            return;
        }
        ...
        try
        {
            unsafe
            {
                ...
                _originalDispatchSbUpdateObjectPtr = MinHook.Hook(_targetAddress, (IntPtr)pDetour);
            }

            IsInstalled = true;
            _statusMessage = $"Hooked CM_Physics::DispatchSB_UpdateObject @ 0x{_targetAddress.ToInt32():X8}.";
            log?.Invoke($"Compat: update-object hook ready - DispatchSB_UpdateObject=0x{_targetAddress.ToInt32():X8}");
        }
        catch (Exception ex)
        {
            _statusMessage = ex.Message;
            log?.Invoke($"Compat: update-object hook failed - {ex.Message}");
        }
    }
    ...
    private static unsafe uint DispatchSbUpdateObjectDetour(IntPtr smartBoxPtr, IntPtr blob)
    {
        ...
        if (count <= 8)
            EntryPoint.Log($"Compat: update object #{count} id=0x{objectId:X8} status={status}");
